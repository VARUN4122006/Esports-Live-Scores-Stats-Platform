...
const ValorantPage = ({ onBack }) => {
    ...
    if (error) {
        return (
            <div className="min-h-screen bg-[#0a0a0c] text-white flex flex-col items-center justify-center p-6 text-center">
                ...
                <h2 className="text-3xl font-black uppercase tracking-tighter mb-2 italic">Operation Failed</h2>
    ...
    return (
        <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="min-h-screen bg-[#0a0a0c] text-white pt-24 pb-20 relative overflow-hidden font-outfit"
            style={{ backgroundImage: 'radial-gradient(circle at 50% -20%, rgba(255, 70, 85, 0.15), transparent)' }}
        >
            ...
            <motion.div
                ...
                style={{ backgroundImage: 'url("https://images8.alphacoders.com/131/1318451.png")' }}
            >
            ...
            <div className="absolute inset-0 z-0 opacity-20 pointer-events-none">
                <div className="absolute top-20 -left-20 w-[600px] h-[600px] bg-[#ff4655] rounded-full blur-[150px] animate-pulse" />
                <div className="absolute bottom-40 -right-20 w-[500px] h-[500px] bg-[#00f2ff] rounded-full blur-[120px] opacity-40" />
            </div>
                ...
                <div className="sticky top-20 z-40 bg-black/60 backdrop-blur-2xl border border-white/10 rounded-2xl p-2 mb-10 flex flex-wrap gap-2 shadow-2xl">
                    {navItems.map((item) => (
                        <button
                            ...
                            className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold uppercase text-xs tracking-widest transition-all ${activeSection === item.id ? 'bg-[#ff4655] text-white shadow-[0_0_20px_rgba(255,70,85,0.4)]' : 'text-gray-400 hover:text-white hover:bg-white/5'} ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {valorantData.liveMatches.map((match) => (
                ...
                >
                    ...
                    <div className="absolute top-0 right-0 p-6"><span className="flex items-center gap-2 px-3 py-1 bg-[#ff4655] rounded-full text-[10px] font-black uppercase animate-pulse shadow-[0_0_15px_rgba(255,70,85,0.5)]"><Radio className="w-3 h-3" /> Live</span></div>
                    <div className="mb-6"><h3 className="text-[#ff4655] font-black uppercase text-sm tracking-widest mb-1 italic">{match.tournament}</h3><p className="text-gray-500 text-xs font-bold uppercase">{match.round} • {match.map}</p></div>
                    <div className="space-y-4">
                        {match.teams.map((team, idx) => (
                            <div key={team.name} className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-6">
                                    ...
                                    <div className="text-right min-w-[40px]"><p className="text-[10px] text-gray-500 uppercase font-bold">Rank</p><p className={`font-black text-2xl tracking-tighter ${idx === 0 ? 'text-[#ff4655] italic' : ''}`}>#{team.rank}</p></div>
                                </div>
                            </div>
                        ))}
                    </div>
                    <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between group-hover:text-[#ff4655] transition-colors"><span className="text-[10px] font-black uppercase tracking-widest text-[#00f2ff]">View Tactical Center</span><ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" /></div>
                </motion.div>
            ))}
        </div>
    );
        ...
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 30 }} className="relative w-full max-w-5xl bg-[#0a0a0c] border border-white/10 rounded-[2.5rem] overflow-hidden shadow-[0_0_100px_rgba(255,70,85,0.15)] flex flex-col md:flex-row h-full max-h-[85vh]">
            <div className="p-10 bg-[#ff4655] text-white w-full md:w-80 shrink-0 flex flex-col justify-between shadow-[inset_-20px_0_40px_rgba(0,0,0,0.1)]">
                <div>
                    ...
                    <div className="bg-black text-[#ff4655] px-3 py-1 inline-block rounded-full text-[10px] font-black uppercase tracking-widest mb-6 border border-[#ff4655]/20">Encrypted Stream</div>
            ...
            <div className="flex-1 p-10 overflow-y-auto bg-black/40 backdrop-blur-md">
                <div className="flex items-center justify-between mb-8"><h3 className="text-2xl font-black uppercase tracking-tight italic">Scoreboard</h3><BarChart3 className="w-6 h-6 text-[#ff4655]" /></div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {valorantData.tournaments.map((tourney) => (
            <div key={tourney.id} className="group bg-[#111115] border border-white/5 rounded-3xl p-8 hover:bg-[#ff4655]/5 hover:border-[#ff4655]/20 transition-all shadow-xl">
                ...
                <h3 className="text-2xl font-black uppercase tracking-tighter mb-2 group-hover:text-[#ff4655] transition-colors italic">{tourney.name}</h3>
            ...
            {valorantData.teams.map((team) => {
                ...
                return (
                    <div key={team.id} className="bg-[#111115] border border-white/5 rounded-3xl p-8 shadow-xl">
                        <div className="flex items-center gap-6 mb-8"><div className="w-20 h-20 bg-white/5 border border-white/10 rounded-[2rem] flex items-center justify-center text-3xl font-black italic shadow-[0_0_30px_rgba(255,70,85,0.1)]" style={{ color: team.color }}>{team.logo}</div><div className="flex-1"><h3 className="text-3xl font-black uppercase tracking-tighter italic">{team.name}</h3><div className="flex gap-2 mt-2">{team.achievements.slice(0, 1).map((ach) => (<span key={ach} className="text-[9px] font-black uppercase tracking-widest px-3 py-1 bg-[#ff4655]/10 border border-[#ff4655]/20 rounded-full text-[#ff4655] italic shadow-sm">🏆 {ach}</span>))}</div></div>
                            <motion.button
...
const StatsSection = () => (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-2 space-y-6">
            <h3 className="text-2xl font-black uppercase tracking-tight flex items-center gap-3 italic"><Swords className="text-[#ff4655]" /> Impact Leaderboard</h3>
