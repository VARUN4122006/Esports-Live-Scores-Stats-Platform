...
import { Trophy, Users, User, BarChart3, Radio, ChevronRight, MapPin, Swords, Target, Zap, AlertTriangle, RefreshCw, Heart, UserPlus } from 'lucide-react';
...
const LoLPage = ({ onBack }) => {
    ...
    const loadData = () => {
        ...
        setTimeout(() => {
            if (Math.random() < 0.02) {
                setError("Summoner's Rift synchronization failed: Riot HQ unreachable.");
                setIsLoading(false);
                return;
            }
            setIsLoading(false);
    ...
    if (error) {
        return (
            <div className="min-h-screen bg-[#05080a] text-white flex flex-col items-center justify-center p-6 text-center">
...
const SkeletonLoader = ({ type }) => (
    <div className={`grid ${type === 'stats' ? 'grid-cols-1 xl:grid-cols-3' : 'grid-cols-1 md:grid-cols-2'} gap-6`}>
        {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-3xl p-8 animate-pulse shadow-xl">
                ...
                <div className="space-y-3">
                    <div className="w-full h-12 bg-white/5 rounded-xl shadow-inner" />
                </div>
            </div>
        ))}
    </div>
...
const LiveMatches = ({ onSelectMatch }) => (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {lolData.liveMatches.map((match) => (
            <motion.div
                key={match.id}
                whileHover={{ y: -8, scale: 1.01 }}
                className="group relative bg-[#090b12] border border-white/5 rounded-3xl p-8 hover:border-blue-500/40 transition-all cursor-pointer overflow-hidden shadow-2xl"
...
const MatchCenter = ({ match, onClose }) => (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 lg:p-10">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 30 }} className="relative w-full max-w-5xl bg-[#080a12] border border-white/10 rounded-[2.5rem] overflow-hidden shadow-[0_0_100px_rgba(59,130,246,0.15)] flex flex-col md:flex-row h-full max-h-[85vh]">
            <div className="p-10 bg-blue-600 text-white w-full md:w-80 shrink-0 flex flex-col justify-between shadow-[inset_-20px_0_40px_rgba(0,0,0,0.1)]">
                ...
                <div className="mt-10 pt-6 border-t border-white/20">
                    <p className="text-[10px] uppercase font-bold opacity-70 mb-2">MVP Candidate</p>
                    <div className="flex items-center gap-4"><div className="w-12 h-12 bg-black rounded-xl flex items-center justify-center font-black text-blue-500 shadow-lg">FA</div><div><p className="font-black uppercase text-sm">Faker</p><p className="text-[10px] uppercase font-bold opacity-70 italic text-white/60">T1 Esports</p></div></div>
                </div>
            </div>
                ...
                <div className="space-y-3">
                    {[1, 2, 3, 4, 5, 6].map((i) => (
                        <div key={i} className="flex items-center justify-between p-5 bg-white/5 border border-white/5 rounded-2xl hover:border-blue-500/20 transition-all group">
                            <div className="flex items-center gap-6"><span className={`text-xl font-black ${i === 1 ? 'text-blue-500 italic scale-110' : 'text-gray-500'}`}>#{i}</span><div className="w-10 h-10 bg-white/5 border border-white/10 rounded-lg flex items-center justify-center font-black text-[10px] shadow-inner">C-{i}</div><div><p className="font-bold uppercase tracking-wide group-hover:text-blue-500 transition-colors text-lg">Champ Alpha {i}</p><p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest italic flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-blue-500" /> CS/Min: {(8.5 + i * 0.2).toFixed(1)}</p></div></div>
                            <div className="flex items-center gap-8"><div className="text-right"><p className="text-[10px] text-gray-500 uppercase font-bold">K/D/A</p><p className="font-black text-xl italic">{8 - i}/{2 + i}/{15}</p></div><div className="text-right min-w-[70px] bg-blue-500/10 border border-blue-500/20 px-4 py-2 rounded-xl"><p className="text-[10px] text-blue-500 uppercase font-bold">Gold</p><p className="font-black text-xl text-blue-500 italic">{12 - i * 0.5}k</p></div></div>
                        </div>
                    ))}
                </div>
            </div>
                    ...
                    <div key={team.id} className="bg-[#090b12] border border-white/5 rounded-3xl p-8 shadow-xl">
                        <div className="flex items-center gap-6 mb-8"><div className="w-20 h-20 bg-white/5 border border-white/10 rounded-[2rem] flex items-center justify-center text-3xl font-heading font-black italic shadow-[0_0_30px_rgba(59,130,246,0.1)]" style={{ color: team.color }}>{team.logo}</div><div className="flex-1"><h3 className="text-3xl font-heading font-black uppercase tracking-tighter italic">{team.name}</h3><div className="flex gap-2 mt-2">{team.achievements.slice(0, 1).map((ach) => (<span key={ach} className="text-[9px] font-button font-black uppercase tracking-widest px-3 py-1 bg-blue-500/10 border border-blue-500/20 rounded-full text-blue-500 italic shadow-sm">🏆 {ach}</span>))}</div></div>
                            <motion.button
                                ...
                                whileTap={{ scale: 0.95 }}
                                onClick={() => toggleFollow({ ...team, game: 'LoL' })}
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-8">
                            ...
                            <div className="bg-white/5 border border-white/5 p-4 rounded-2xl text-center shadow-inner"><p className="text-[9px] text-gray-500 uppercase font-bold mb-1 tracking-widest leading-none">Avg Kills</p><p className="text-xl font-black italic">{team.stats.avgKills}</p></div>
            ...
            <div key={player.id} className="bg-[#090b12] border border-white/5 rounded-3xl p-8 transition-all flex flex-col xl:flex-row gap-8 shadow-xl border-l-[3px] border-l-blue-500 hover:border-blue-500/40 hover:scale-[1.01] transition-all">
                ...
                <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        ...
                        <div className="text-center xl:text-left"><p className="text-[9px] text-blue-500 uppercase font-black tracking-widest flex items-center gap-1 justify-center xl:justify-start mb-1 leading-none"><Trophy className="w-3 h-3" /> MVP</p><p className="text-md font-black italic text-blue-500">{player.stats.mvp}</p></div>
                    </div>
                </div>
            </div>
        ))}
            ...
            <div className="bg-[#090b12] border border-white/5 rounded-[2.5rem] p-8 shadow-2xl">
                {[1, 2, 3].map((i) => (
                    <div key={i} className={`flex items-center justify-between py-6 ${i !== 3 ? 'border-b border-white/5' : ''} group cursor-default hover:bg-white/5 transition-colors px-4 rounded-2xl`}>
                        <div className="flex items-center gap-6"><span className={`text-2xl font-black ${i === 1 ? 'text-blue-500 italic scale-125' : 'text-gray-500'}`}>0{i}</span><div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center font-black border border-white/5 text-xs">LOL</div><div><p className="font-black uppercase tracking-tight text-xl leading-none italic group-hover:text-blue-500 transition-colors">Legend {i}</p><p className="text-[10px] text-gray-500 font-black uppercase tracking-widest mt-1 italic">Pro Circuit</p></div></div>
                        <div className="text-right"><p className="text-[9px] text-gray-500 uppercase font-bold mb-1 tracking-widest">Avg CS/M</p><p className="font-black text-2xl text-blue-500 tracking-tighter italic">{(9.5 - i * 0.3).toFixed(1)}</p></div>
                    </div>
                ))}
            </div>
        </div>
            ...
            <div className="bg-[#090b12] border border-white/10 rounded-[2.5rem] p-8 space-y-8 shadow-2xl">
                {[1, 2, 3, 4, 5].map((i) => (
                    <div key={i} className="flex flex-col gap-2">
                        <div className="flex justify-between items-center"><span className="font-black text-[10px] uppercase tracking-widest italic">{i === 1 ? 'Challenger' : i === 2 ? 'Grandmaster' : 'Master'}</span><span className="font-black text-xs text-blue-500 italic">{98 - i * i}%</span></div>
                        <div className="h-2 bg-white/5 border border-white/5 rounded-full overflow-hidden shadow-inner"><div className="h-full bg-gradient-to-r from-blue-500 via-blue-200 to-transparent shadow-[0_0_10px_rgba(59,130,246,0.4)]" style={{ width: `${98 - i * i}%` }} /></div>
                    </div>
                ))}
            </div>
        </div>
