    ...
    if (error) {
        return (
            <div className="min-h-screen bg-bg-dark text-white flex flex-col items-center justify-center p-6 text-center font-sans">
                ...
                <h2 className="text-3xl font-bold uppercase tracking-tighter mb-2 italic">Aegis connection Lost</h2>
                ...
                <div className="flex gap-4">
                    ...
                    <button onClick={loadData} className="px-8 py-3 bg-gold hover:bg-white hover:text-black text-bg-dark font-bold uppercase tracking-widest rounded-full transition-all flex items-center gap-2 shadow-[0_0_30px_rgba(212,175,55,0.3)]"><RefreshCw className="w-4 h-4" /> Re-summon</button>
                </div>
            </div>
        );
    }
                ...
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h1 className="text-4xl md:text-6xl font-black uppercase tracking-tighter flex items-center gap-3 italic">
                            DEFENSE OF THE <span className="text-gold not-italic">ANCIENTS 2</span>
                        </h1>
        ...
        {dota2Data.liveMatches.map((match) => (
            ...
            >
                ...
                <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between group-hover:text-gold transition-colors">
                    <span className="text-[10px] font-bold uppercase tracking-widest">View Aegis Center</span>
        ...
        <div className="relative w-full max-w-5xl bg-bg-dark border border-white/10 rounded-[2.5rem] overflow-hidden shadow-[0_0_100px_rgba(212,175,55,0.15)] flex flex-col md:flex-row h-full max-h-[85vh] animate-scale-in">
            <div className="p-10 bg-gradient-to-br from-gold to-amber text-bg-dark w-full md:w-80 shrink-0 flex flex-col justify-between shadow-[inset_-20px_0_40px_rgba(0,0,0,0.1)]">
                <div>
                    ...
                    <div className="bg-bg-dark text-gold px-3 py-1 inline-block rounded-full text-[10px] font-bold uppercase tracking-widest mb-6 border border-gold/20">The International</div>
                    <div className="space-y-4 text-bg-dark/80">
                        ...
                        <div className="flex items-center gap-3 font-bold"><Radio className="w-4 h-4" /><span className="text-sm font-black uppercase tracking-tight">Phase: Late Game</span></div>
                    </div>
                </div>
                <div className="mt-10 pt-6 border-t border-bg-dark/20">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-bg-dark rounded-xl flex items-center justify-center font-black text-gold shadow-lg">DS</div>
                        <div><p className="font-black uppercase text-sm">Yatoro</p><p className="text-[10px] uppercase font-bold opacity-70 italic font-black">Spirit</p></div>
                    </div>
                </div>
            </div>
                    ...
                    {[1, 2, 3, 4, 5, 6].map((i) => (
                        <div key={i} className="flex items-center justify-between p-5 bg-white/5 border border-white/5 rounded-2xl hover:border-gold/20 transition-all group">
                            <div className="flex items-center gap-6">
                                ...
                                <div className="w-10 h-10 bg-white/5 border border-white/10 rounded-lg flex items-center justify-center font-black text-[10px] shadow-inner">H-{i}</div>
                                <div>
                                    <p className="font-bold uppercase tracking-wide group-hover:text-gold transition-colors text-lg">Hero Alpha {i}</p>
                                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest italic flex items-center gap-2">
                                        <span className="w-1.5 h-1.5 rounded-full bg-gold" /> GPM: {600 + i * 20}
                                    </p>
                                </div>
                            </div>
                            <div className="flex items-center gap-8">
                                <div className="text-right"><p className="text-[10px] text-gray-500 uppercase font-bold">K/D/A</p><p className="font-black text-xl italic">{15 - i}/{5 + i}/{12}</p></div>
                                <div className="text-right min-w-[70px] bg-gold/10 border border-gold/20 px-4 py-2 rounded-xl">
                                    <p className="text-[10px] text-gold uppercase font-bold">Impact</p>
                                    <p className="font-black text-xl text-gold italic">{(4.5 - i * 0.2).toFixed(1)}</p>
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                return (
                    <div key={team.id} className="bg-card border border-white/5 rounded-3xl p-8 shadow-xl">
                        <div className="flex items-center gap-6 mb-8">
                            <div className="w-20 h-20 bg-white/5 border border-white/10 rounded-[2rem] flex items-center justify-center text-3xl font-black italic shadow-[0_0_30px_rgba(225,6,0,0.1)]" style={{ color: team.color }}>{team.logo}</div>
                            <div className="flex-1">
                                ...
                                <div className="flex gap-2 mt-2">
                                    {team.achievements.slice(0, 1).map((ach) => (
                                        <span key={ach} className="text-[9px] font-bold uppercase tracking-widest px-3 py-1 bg-[#FF1E1E]/10 border border-[#FF1E1E]/20 rounded-full text-[#FF1E1E] italic shadow-sm">🏆 {ach}</span>
                                    ))}
                                </div>
                            </div>
                            <button
                                ...
                                className={`shrink-0 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-200 border ${followed
                                    ? 'bg-[#FF1E1E] text-white shadow-[0_0_30px_rgba(255,30,30,0.8)] border-[#FF1E1E]'
                ...
                <div key={player.id} className="bg-card border border-white/5 rounded-3xl p-8 transition-all flex flex-col xl:flex-row gap-8 shadow-xl border-l-[3px] border-l-gold hover:-translate-y-1 group">
                    ...
                    <div className="flex-1">
                        <div className="mb-6 flex flex-col xl:flex-row xl:items-end justify-between gap-4">
                            ...
                            <div className="flex items-center gap-4">
                                <button
                                    onClick={() => toggleFollowPlayer({ ...player, game: 'Dota 2' })}
                                ...
                                <div className="flex gap-1 h-8">
                                    {player.performance.map((val, idx) => (
                                        <div key={idx} className="w-2.5 bg-gold/20 rounded-t-sm self-end hover:bg-gold transition-colors" style={{ height: `${(val / 50) * 100}%` }} />
                                    ))}
                                </div>
                            </div>
                        </div>
...
const StatsSection = () => (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-2 space-y-6">
            <h3 className="text-2xl font-black uppercase tracking-tight flex items-center gap-3 italic"><Swords className="text-crimson" /> Professional GPM</h3>
                ...
                {[1, 2, 3].map((i) => (
                    <div key={i} className={`flex items-center justify-between py-6 ${i !== 3 ? 'border-b border-white/5' : ''} group cursor-default hover:bg-white/5 transition-colors px-4 rounded-2xl`}>
                        <div className="flex items-center gap-6">
                            ...
                            <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center font-black border border-white/5 text-xs">TI</div>
                            <div>
                                <p className="font-black uppercase tracking-tight text-xl leading-none italic group-hover:text-crimson transition-colors">Summoner {i}</p>
                                <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest mt-1 italic">Pro Circuit</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <p className="text-[9px] text-gray-500 uppercase font-bold mb-1 tracking-widest">Avg GPM</p>
                            <p className="font-black text-2xl text-crimson tracking-tighter italic">{750 - i * 25}</p>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="bg-card border border-white/10 rounded-[2.5rem] p-8 space-y-8 shadow-2xl">
                {[1, 2, 3, 4, 5].map((i) => (
                    <div key={i} className="flex flex-col gap-2">
                        <div className="flex justify-between items-center"><span className="font-black text-[10px] uppercase tracking-widest italic">{i === 1 ? 'Immortal' : i === 2 ? 'Divine' : 'Ancient'}</span><span className="font-black text-xs text-crimson italic">{98 - i * i}%</span></div>
                        <div className="h-2 bg-white/5 border border-white/5 rounded-full overflow-hidden shadow-inner">
                            <div className="h-full bg-gradient-to-r from-[#FF1E1E] via-amber to-transparent shadow-[0_0_10px_rgba(225,6,0,0.4)]" style={{ width: `${98 - i * i}%` }} />
                        </div>
                    </div>
                ))}
            </div>
