    ...
    if (error) {
        return (
            <div className="min-h-screen bg-bg-dark text-white flex flex-col items-center justify-center p-6 text-center font-sans">
                <div className="w-20 h-20 bg-crimson/10 rounded-3xl flex items-center justify-center mb-6 border border-crimson/20 shadow-[0_0_30px_rgba(225,6,0,0.1)]">
                    <AlertTriangle className="w-10 h-10 text-crimson" />
                </div>
                <h2 className="text-3xl font-bold uppercase tracking-tighter mb-2 italic">Signal Lost</h2>
                ...
                <div className="flex gap-4">
                    ...
                    <button onClick={loadData} className="px-8 py-3 bg-crimson hover:bg-white hover:text-black text-white font-bold uppercase tracking-widest rounded-full transition-all flex items-center gap-2 shadow-[0_0_30px_rgba(225,6,0,0.3)]"><RefreshCw className="w-4 h-4" /> Re-link</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-bg-dark text-white pt-24 pb-20 relative overflow-hidden font-sans animate-fade-in">
            ...
            <div className="absolute inset-0 pointer-events-none">
                <div className="absolute top-[-20%] right-[-10%] w-[800px] h-[800px] bg-purple/[0.05] blur-[150px] rounded-full" />
                <div className="absolute bottom-[-15%] left-[-10%] w-[600px] h-[600px] bg-crimson/[0.05] blur-[130px] rounded-full" />
            </div>

            <div className="container mx-auto px-6 relative z-10">
                {/* Header */}
                <div className="flex items-center gap-4 mb-8">
                    <button onClick={onBack} className="w-12 h-12 flex items-center justify-center rounded-full bg-white/5 border border-white/10 hover:bg-crimson/10 hover:border-crimson/30 transition-all duration-300 group">
                        <ChevronRight className="w-6 h-6 rotate-180 text-gray-400 group-hover:text-crimson transition-colors" />
                    </button>
                    <div>
                        <h1 className="text-4xl md:text-6xl font-black uppercase tracking-tighter flex items-center gap-3 italic">
                            COUNTER-STRIKE <span className="text-crimson not-italic">2</span>
                        </h1>
                    ...
                    {navItems.map((item) => (
                        <button
                            ...
                            className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold uppercase text-xs tracking-widest transition-all duration-300 ${activeSection === item.id
                                ? 'bg-crimson text-white shadow-[0_0_20px_rgba(225,6,0,0.4)]'
...
const MatchCenter = ({ match, onClose }) => (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 lg:p-10 animate-fade-in">
        ...
        <div className="relative w-full max-w-5xl bg-bg-dark border border-white/10 rounded-[2.5rem] overflow-hidden shadow-[0_0_100px_rgba(255,215,0,0.15)] flex flex-col md:flex-row h-full max-h-[85vh] animate-scale-in">
            <div className="p-10 bg-gold text-white w-full md:w-80 shrink-0 flex flex-col justify-between shadow-[inset_-20px_0_40px_rgba(0,0,0,0.1)]">
                <div>
                    ...
                    <div className="bg-black text-gold px-3 py-1 inline-block rounded-full text-[10px] font-bold uppercase tracking-widest mb-6 border border-gold/20">Active Defusal</div>
                    <div className="space-y-4 text-white/80">
                        ...
                        <div className="flex items-center gap-3 font-bold"><Radio className="w-4 h-4" /><span className="text-sm font-black uppercase tracking-tight">Half 2 / Overtime</span></div>
                    </div>
                </div>
                <div className="mt-10 pt-6 border-t border-white/20">
                    <p className="text-[10px] uppercase font-bold opacity-70 mb-2">Round MVP</p>
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-black rounded-xl flex items-center justify-center font-black text-gold shadow-lg">CS</div>
                        <div><p className="font-black uppercase text-sm">ZywOo</p><p className="text-[10px] uppercase font-bold opacity-70 italic">Vitality</p></div>
                    </div>
                </div>
            </div>
                    ...
                    {[1, 2, 3, 4, 5, 6].map((i) => (
                        <div key={i} className="flex items-center justify-between p-5 bg-white/5 border border-white/5 rounded-2xl hover:border-gold/20 transition-all group">
                            <div className="flex items-center gap-6">
                                ...
                                <div className="w-10 h-10 bg-white/5 border border-white/10 rounded-lg flex items-center justify-center font-black text-[10px] shadow-inner">G-{i}</div>
                                <div>
                                    <p className="font-bold uppercase tracking-wide group-hover:text-gold transition-colors text-lg">Gunner Alpha {i}</p>
                                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest italic flex items-center gap-2">
                                        <span className="w-1.5 h-1.5 rounded-full bg-gold" /> ADR: {90 + i * 2}
                                    </p>
                                </div>
                            </div>
                            <div className="flex items-center gap-8">
                                <div className="text-right"><p className="text-[10px] text-gray-500 uppercase font-bold">K/D</p><p className="font-black text-xl italic">{(1.5 - i * 0.1).toFixed(2)}</p></div>
                                <div className="text-right min-w-[70px] bg-gold/10 border border-gold/20 px-4 py-2 rounded-xl">
                                    <p className="text-[10px] text-gold uppercase font-bold">Rating</p>
                                    <p className="font-black text-xl text-gold italic">{(1.4 - i * 0.05).toFixed(2)}</p>
                                </div>
                            </div>
                        </div>
                    ))}
        ...
        {cs2Data.tournaments.map((tourney) => (
            <div key={tourney.id} className="group bg-card border border-white/5 rounded-3xl p-8 hover:bg-gold/5 hover:border-gold/20 transition-all duration-300 shadow-xl">
                <div className="flex justify-between items-start mb-6">
                    ...
                    <span className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${tourney.status === 'Ongoing' ? 'bg-gold text-white border-gold transition-colors shadow-[0_0_10px_rgba(255,215,0,0.3)]' : 'bg-white/5 text-white border-white/10'}`}>
                        {tourney.status}
                    </span>
                </div>
                ...
                return (
                    <div key={team.id} className="bg-card border border-white/5 rounded-3xl p-8 shadow-xl">
                        <div className="flex items-center gap-6 mb-8">
                            <div className="w-20 h-20 bg-white/5 border border-white/10 rounded-[2rem] flex items-center justify-center text-3xl font-black italic shadow-[0_0_30px_rgba(255,215,0,0.1)]" style={{ color: team.color }}>{team.logo}</div>
                            ...
                            <button
                                ...
                                className={`shrink-0 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-200 border ${followed
                                    ? 'bg-gold text-bg-dark shadow-[0_0_30px_rgba(255,215,0,0.8)] border-gold'
                ...
                <div key={player.id} className="bg-card border border-white/5 rounded-3xl p-8 transition-all flex flex-col xl:flex-row gap-8 shadow-xl border-l-[3px] border-l-gold hover:-translate-y-1 group">
                    ...
                    <div className="flex-1">
                        <div className="mb-6 flex flex-col xl:flex-row xl:items-end justify-between gap-4">
                            ...
                            <div className="flex items-center gap-4">
                                <button
                                    onClick={() => toggleFollowPlayer({ ...player, game: 'CS2' })}
                                    className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all border ${isFollowingPlayer(player.name)
                                        ? 'bg-gold text-bg-dark border-gold shadow-[0_0_20px_rgba(255,215,0,0.4)]'
                                        : 'bg-white/5 border-white/10 text-gray-400 hover:text-white hover:border-white/20'
                                        }`}
                                >
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            ...
                            <div className="text-center xl:text-left"><p className="text-[9px] text-gray-500 uppercase font-black tracking-widest flex items-center gap-1 justify-center xl:justify-start mb-1 leading-none"><Zap className="w-3 h-3" /> ADR</p><p className="text-md font-black italic">{player.stats.matches}</p></div>
...
const StatsSection = () => (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-2 space-y-6">
            <h3 className="text-2xl font-black uppercase tracking-tight flex items-center gap-3 italic"><Swords className="text-crimson" /> Professional ADR</h3>
            <div className="bg-card border border-white/5 rounded-[2.5rem] p-8 shadow-2xl">
                {[1, 2, 3].map((i) => (
                    <div key={i} className={`flex items-center justify-between py-6 ${i !== 3 ? 'border-b border-white/5' : ''} group cursor-default hover:bg-white/5 transition-colors px-4 rounded-2xl`}>
                        <div className="flex items-center gap-6">
                            <span className={`text-2xl font-black ${i === 1 ? 'text-crimson italic scale-125' : 'text-gray-500'}`}>0{i}</span>
                            <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center font-black border border-white/5 text-xs">HLTV</div>
                            <div>
                                <p className="font-black uppercase tracking-tight text-xl leading-none italic group-hover:text-crimson transition-colors">Sharpshooter {i}</p>
                                <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest mt-1 italic">Major Legends</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <p className="text-[9px] text-gray-500 uppercase font-black mb-1 tracking-widest">HLTV Rating</p>
                            <p className="font-black text-2xl text-crimson tracking-tighter italic">{(1.45 - i * 0.1).toFixed(2)}</p>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="bg-card border border-white/10 rounded-[2.5rem] p-8 space-y-8 shadow-2xl">
                {[1, 2, 3, 4, 5].map((i) => (
                    <div key={i} className="flex flex-col gap-2">
                        <div className="flex justify-between items-center"><span className="font-black text-[10px] uppercase tracking-widest italic">{i === 1 ? 'Global Elite' : i === 2 ? 'Supreme Master' : 'Legendary Eagle'}</span><span className="font-black text-xs text-crimson italic">{98 - i * i}%</span></div>
                        <div className="h-2 bg-white/5 border border-white/5 rounded-full overflow-hidden shadow-inner">
                            <div className="h-full bg-gradient-to-r from-crimson via-purple to-transparent shadow-[0_0_10px_rgba(225,6,0,0.4)]" style={{ width: `${98 - i * i}%` }} />
                        </div>
                    </div>
                ))}
            </div>
